...
export const LogoLoop = memo(({
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: Key) => {
    if (renderItem) return <li className="logoloop__item" key={key as string} role="listitem">{renderItem(item, key)}</li>;

    const isNode = 'node' in item;
    const content = isNode ? (
      <span className="logoloop__node" aria-hidden={!!item.href && !(item as NodeLogoItem).ariaLabel}>
